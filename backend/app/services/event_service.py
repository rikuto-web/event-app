...
class EventService:
    ...
    def _detail_response(self, row, *, user_id: UUID | None = None) -> EventDetailResponse:
        my_participation = None
        if user_id is not None:
            my_participation = self.events.get_participation_status(row.event.id, user_id)
    ...
    def _detail_for_user(self, user_id: UUID, event_id: UUID) -> EventDetailResponse:
        row = self.events.get_for_member(user_id, event_id)
        if row is None:
            raise self._event_not_found()
        return self._detail_response(row, user_id=user_id)
    ...
    def get_event(self, user_id: UUID, event_id: UUID) -> EventDetailResponse:
        return self._detail_for_user(user_id, event_id)
    ...
    async def update_event(self, user_id: UUID, event_id: UUID, data: EventUpdateRequest) -> EventDetailResponse:
        ...
        detail = self._detail_for_user(user_id, event_id)
